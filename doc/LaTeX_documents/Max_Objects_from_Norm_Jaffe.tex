...
% be sure to set the boolean 'generatingPDF' to false.
\usepackage[dvips,
colorlinks=true,
linkcolor=webgreen,
filecolor=webbrown,
...
% Control the code, depending on whether PDF is being generated:
\newboolean{generatingPDF}
\setboolean{generatingPDF}{true}
...
\ifthenelse{\boolean{generatingPDF}}%
  {\newcommand{\entityNameTagINT}[3]{%  command if generatingPDF
  \ifthenelse{\equal{#1}{D}}%
    {\textit{\color{webgreen}#3}}%   if first argument is 'D'
    {\ifthenelse{\equal{#1}{E}}%
      {}%  if first argument is 'E'
      {\ifthenelse{\equal{#1}{S}}%
        {\textit{#3}}%  if first argument is 'S'
        {\hyperlink{hyper.#2.#3}{\textit{#3}}}}}}}%  if first argument isn't 'D', 'E', or 'S'
  {\newcommand{\entityNameTagINT}[3]{%  command if not generatingPDF
  \ifthenelse{\equal{#1}{D}}%
    {\textit{\color{webgreen}#3}}%  if first argument is 'D'
    {\ifthenelse{\equal{#1}{E}}%
      {}%  if first argument is 'E'
      {\textit{#3}}}}}%  if first argument isn't 'D' or 'E'

\ifthenelse{\boolean{generatingPDF}}%
  {\newcommand{\entityNameIndexINT}[3]{%  command if generatingPDF
   \ifthenelse{\equal{#1}{D}}%
  ...
  {\newcommand{\entityNameIndexINT}[3]{%  command if not generatingPDF
   \ifthenelse{\equal{#1}{D}}%
...
%  generatingPDF
%   'D' \textit{\color{webgreen}#3}  \label{#2:#3}  {}
%   'E' {}  {}  {}
%   'R' \hyperlink{hyper.#2.#3}{\textit{#3}}  \index{#2!#3}  \ref{#2:#3}
%   'S' \textit{#3}  \index{#2!#3}  {}
%   'X' \hyperlink{hyper.#2.#3}{\textit{#3}}  {}  {}
%  not generatingPDF
%   'D' \textit{\color{webgreen}#3}  \index{#2!#3|(textbf}\label{#2:#3}  {}
%   'E' {}  \index{#2!#3|)textbf}  {}
%   'R' \textit{#3}  \index{#2!#3}  \ref{#2:#3}
%   'S' \textit{#3}  \index{#2!#3}  {}
%   'X' \textit{#3}  {}  {}
\ifthenelse{\boolean{generatingPDF}}%
  {\newcommand{\companyReference}[2]{\href{#1}{#2}}}%  command if generatingPDF
  {\newcommand{\companyReference}[2]{#2}}% command if not generatingPDF
...
\ifthenelse{\boolean{generatingPDF}}%
  {\newcommand{\entityReference}[2]{\entityName[R]{#1}{#2}}}%  command if generatingPDF
  {\newcommand{\entityReference}[2]{\entityName[R]{#1}{#2} \vpageref[(][(]{#1:#2})}}%  command if not generatingPDF

\newcommand{\objReference}[1]{\entityReference{Objects}{#1}}
\newcommand{\pluginReference}[2]{\entityReference{#1}{#2}}

\ifthenelse{\boolean{generatingPDF}}%
  {\newcommand{\objImageReference}[1]{\ref{#1}}}%  command if generatingPDF
  {\newcommand{\objImageReference}[1]{\vref{#1}}}%  command if not generatingPDF
...
\newcommand*{\objArgType}[1]{\textit{#1}}
\newcommand*{\objIOType}[1]{\textbf{#1}}

\newcommand*{\compLang}[1]{\emph{#1}}
...
\newcommand{\objListIOItem}[2]{\item \objIOType{#1}, #2}
...
\title{\cal\Huge\textit{Max~Objects~from~Norm~Jaffe}\\
\vspace{1ex}
